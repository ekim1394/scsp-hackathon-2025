...
def update_user_organization(user_id: int, user_update: UserUpdate, session: Session = Depends(get_session)):
    user_to_update = session.exec(select(User).where(User.id == user_id)).first()
    if not user_to_update:
        raise HTTPException(status_code=404, detail="User not found")
    
    user_to_update.organization = user_update.organization if hasattr(user_update, 'organization') else None
